 ...
 */
public abstract class KnowledgeHeuristicMulti extends MultiStateHeuristic {
    ...
    protected InteractionHistoryMulti interaction_history;

    public KnowledgeHeuristicMulti(StateObservationMulti stateObs, int playerID){
        ...
        interaction_history = new InteractionHistoryMulti(current_gametick, playerID);
    ...
    private boolean updateInteractionHistory(core.game.Event event, StateObservationMulti stateObs, StateObservationMulti last_stateObs){
    ...
    protected boolean processLastEventsFromState(StateObservationMulti stateObs, StateObservationMulti last_stateObs){
        TreeSet<core.game.Event> events_history = stateObs.getEventsHistory();
        ...
        boolean inthistory_updated = false;
        while (itr.hasNext()){
            core.game.Event event = (core.game.Event) itr.next();
    ...
    protected ArrayList<core.game.Event> getLastGametickEvents(StateObservationMulti stateObs, StateObservationMulti last_stateObs) {
        TreeSet<core.game.Event> events_history = stateObs.getEventsHistory();
        Iterator itr = events_history.descendingSet().iterator();

        ArrayList<core.game.Event> last_gametick_events = new ArrayList<>();
     ...
     */
    protected boolean isNewStypeInteraction(ArrayList<core.game.Event> last_gametick_events, int avatar_stype){
...
//        System.out.println("---------------GAMETICK "+current_gametick+"---------");
    ...
    protected void endOfGameProcess(StateObservationMulti stateObs)
    {
        /* Last updates needed */
    ...
    private void printEvent(core.game.Event event){
        System.out.println(event.gameStep+"-> sp1: "+event.activeTypeId + " sp2: "+event.passiveTypeId + " at "+event.position);
    }

    protected void printEvents(ArrayList<core.game.Event> events){
